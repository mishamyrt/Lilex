...
async def _fix_variable(files: list[str], target_version: float, stat: list[dict]):
    ...
    gen_stat_tables_from_config(stat, fonts, has_italic=True)
    for font in fonts:
...
async def post_process(
    font_map: dict[FontFormat, list[str]],
    ...
    for fmt, files in font_map.items():
        if fmt == FontFormat.TTF:
            ...
            tasks.append(_fix_variable(files, target_version, stat))
